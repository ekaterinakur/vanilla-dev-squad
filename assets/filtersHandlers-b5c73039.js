import{a as R,i as y,b as C,h as P}from"./vendor-478dae0d.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function s(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(i){if(i.ep)return;i.ep=!0;const a=s(i);fetch(i.href,a)}})();const p=R.create({baseURL:"https://your-energy.b.goit.study/api/",timeout:3e3});async function k(e){var t,s;try{return(await p.get(`exercises/${e}`)).data}catch(n){throw new Error(((s=(t=n.response)==null?void 0:t.data)==null?void 0:s.message)||n.message)}}function g(e){return e?e.charAt(0).toUpperCase()+e.slice(1):""}const u="/vanilla-dev-squad/assets/sprite-45ef4b9c.svg";function T(e,t){return`
  ...
  `}function D(e){let t="";for(let s=1;s<=5;s++){const n=s<=Math.floor(e);t+=`
  ...
  `}function A(e){const t=document.getElementById("favoritesBtn");e?t.innerHTML=`
    ...
    `}async function O(e,{email:t,rate:s,review:n}){var i,a;try{return(await p.patch(`exercises/${e}/rating`,{email:t,rate:s,review:n})).data}catch(o){throw new Error(((a=(i=o.response)==null?void 0:i.data)==null?void 0:a.message)||o.message)}}document.getElementById("giveRatingBtn");document.getElementById("closeRatingModal");document.getElementById("closeExerciseModal");const d=document.getElementById("ratingModal");document.getElementById("exerciseModal");const w=document.querySelector("#ratingModal .modal-content"),H=()=>{d.style.display="block",d.classList.remove("modal-closing")},I=()=>{w.classList.add("dispersing"),d.classList.add("modal-closing"),setTimeout(()=>{d.style.display="none",w.classList.remove("dispersing"),d.classList.remove("modal-closing")},600)},K=e=>{const t=document.getElementById("currentRating");t.textContent=`${e}.0`};y.settings({class:"custom-toast",position:"topRight",maxWidth:432,color:"#FFFFFF",titleColor:"#FFFFFF",messageColor:"#FFFFFF",iconColor:"#FFFFFF",displayMode:1});function Z(){document.getElementById("loader").classList.remove("hidden")}function ee(){document.getElementById("loader").classList.add("hidden")}function U(e){y.success({message:e,backgroundColor:"#0C7611"})}function F(e){y.error({message:e,backgroundColor:"#EF4040"})}let v=null;function x(e){v=e,H(),_()}const _=()=>{const e=document.getElementById("closeRatingModal"),t=document.getElementById("ratingForm");e.addEventListener("click",I),t.addEventListener("submit",j),document.querySelectorAll(".stars input").forEach(s=>{s.addEventListener("change",n=>K(n.target.value))})},j=async e=>{e.preventDefault();const t=e.target,s=t.querySelector('input[name="rating"]:checked'),n=t.elements.email.value.trim(),i=t.elements.description.value.trim(),a=parseFloat(s.value||"");if(!a){F("Failed to submit rating.");return}const o={email:n,rate:a,review:i};try{await O(v,o),U("Rating submitted successfully!"),v=null,I()}catch{F("Failed to submit rating.")}};let m;const h="favorites";let c=[],l;async function te(e){l=await k(e),c=localStorage.getItem(h)?JSON.parse(localStorage.getItem(h)):[];const s=c.findIndex(i=>i._id===l._id)!==-1,n=T(l,s);m=C.create(n,{closable:!0,onClose:()=>W()}),m.show(),J()}function J(){document.getElementById("dialogIconClose").addEventListener("click",f),document.addEventListener("keydown",f),document.getElementById("favoritesBtn").addEventListener("click",q),document.getElementById("giveRatingBtn").addEventListener("click",()=>x(l._id))}function W(){document.getElementById("dialogIconClose").removeEventListener("click",f),document.removeEventListener("keydown",f),document.getElementById("favoritesBtn").removeEventListener("click",q),document.getElementById("giveRatingBtn").addEventListener("click",x)}function q(){const e=c.findIndex(t=>t._id===l._id);e!==-1?c.splice(e,1):c.push(l),localStorage.setItem(h,JSON.stringify(c)),A(e!==-1)}function f(e){e instanceof PointerEvent&&m.close(),e instanceof KeyboardEvent&&e.key==="Escape"&&!(e.ctrlKey||e.altKey||e.shiftKey)&&m.close()}const z=document.querySelector(".quote-text-wrapper"),S=document.querySelector(".filter-list"),b=document.querySelector(".pagination-section");function L(e){const t=e.results.map(s=>`
